...
/* React-PDF Image is not a DOM image and does not support an alt prop. */
      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        <View style={styles.meta} wrap={false}>
          <View style={styles.metaItem}>
            ...
            <Text style={styles.metaValue}>{date(note.service_date)}</Text>
          </View>
          <View style={styles.metaItem}>
            <Text style={styles.metaLabel}>Service time</Text>
            <Text style={styles.metaValue}>{note.service_time || "-"}</Text>
          </View>
          <View style={styles.metaItem}>
            <Text style={styles.metaLabel}>Person in charge</Text>
            <Text style={styles.metaValue}>
          ...
          <View style={styles.column}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>
                {[note.contact_name_snapshot, note.contact_position_snapshot]
                  .filter(Boolean)
                  .join(", ") || "-"}
              </Text>
            </View>
        ...
        <View style={styles.section}>
          ...
          <Narrative title="Additional notes">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader} fixed>
              <Text style={[styles.cell, styles.headCell, { width: "23%" }]}>
                Employee
              </Text>
              <Text style={[styles.cell, styles.headCell, { width: "29%" }]}>
              ...
              note.labor.map((item) => (
                <View key={item.id} style={styles.tableRow} wrap={false}>
                  ...
                  <View style={[styles.cell, { width: "29%" }]}>
                    ...
                    <Text style={styles.cellSub}>{item.notes}</Text>
                  </View>
                  <Text
                    style={[styles.cell, styles.numericCell, { width: "12%" }]}
                  >
                    {item.hours}
                  </Text>
                  <Text
                    style={[styles.cell, styles.numericCell, { width: "18%" }]}
                  >
                  ...
                  <Text
                    style={[styles.cell, styles.numericCell, { width: "18%" }]}
                  >
                    {money(amount(item.hours, item.rate), organization)}
                  </Text>
                </View>
              ))
            ) : (
              <Text style={styles.empty}>No labor recorded</Text>
            )}
            <View style={styles.tableFooter} wrap={false}>
              <Text style={[styles.cell, styles.strongCell, { width: "82%" }]}>
                Labor total
              </Text>
              <Text
                style={[
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader} fixed>
              <Text style={[styles.cell, styles.headCell, { width: "30%" }]}>
                Material
              </Text>
              <Text style={[styles.cell, styles.headCell, { width: "18%" }]}>
                Part number
              </Text>
              <Text
                style={[
            ...
            {note.materials.length ? (
              note.materials.map((item) => (
                <View key={item.id} style={styles.tableRow} wrap={false}>
                  <Text
                    style={[styles.cell, styles.strongCell, { width: "30%" }]}
                  >
                    {item.description}
                  </Text>
                  ...
                  <Text
                    style={[styles.cell, styles.numericCell, { width: "20%" }]}
                  >
            ...
            <View style={styles.tableFooter} wrap={false}>
              <Text style={[styles.cell, styles.strongCell, { width: "80%" }]}>
                Materials total
              </Text>
              <Text
                style={[
          ...
          <View style={styles.table}>
            {note.charges.length ? (
              note.charges.map((item) => (
                <View key={item.id} style={styles.chargeRow} wrap={false}>
                  <Text>{item.description}</Text>
                  <Text style={styles.strongCell}>
                    {money(item.amount, organization)}
                  </Text>
                </View>
              ))
            ) : (
              <Text style={styles.empty}>No additional charges</Text>
            )}
            <View style={styles.tableFooter} wrap={false}>
              <Text style={[styles.cell, styles.strongCell, { width: "80%" }]}>
        ...
        <View style={styles.section}>
          ...
          {note.photos.length ? (
              ...
              {note.photos.map((photo) => (
                <View key={photo.id} style={styles.photoCard} wrap={false}>
                  ...
                  <View style={styles.photoCaption}>
                    ...
                    <Text style={styles.photoName}>
                      {photo.caption || photo.name || "Service photo"}
                    </Text>
                    <Text style={styles.photoDate}>
                      {date(photo.created_at, true)}
                    </Text>
                  </View>
                </View>
              ))}
            ...
            <Text style={styles.empty}>No service photos recorded.</Text>
          )}
        </View>

        <View style={styles.section} wrap={false}>
          <SectionHeading number="06">
            Financial summary and payment
          </SectionHeading>
          <View style={styles.financialArea}>
            <View style={styles.payment}>
              <View style={styles.paymentRow}>
                <Text style={styles.paymentLabel}>Status</Text>
                <Text style={[styles.paymentValue, styles.strongCell]}>
            ...
            <View style={styles.totals}>
              ...
              <View style={styles.totalRow}>
                ...
                <Text style={styles.totalValue}>
                  {money(note.subtotal, organization)}
                </Text>
              </View>
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Discount</Text>
                <Text style={styles.totalValue}>
                  - {money(note.discount_amount, organization)}
                </Text>
              </View>
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>
                  Tax ({Number(note.tax_rate) || 0}%)
                </Text>
                <Text style={styles.totalValue}>
                  {money(note.tax_amount, organization)}
                </Text>
              </View>
              <View style={styles.grandTotal}>
                <Text style={styles.grandLabel}>Grand total</Text>
                <Text style={styles.grandValue}>
                  {money(note.grand_total, organization)}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.section} wrap={false}>
          ...
          <View style={styles.acceptance}>
            <Text style={styles.acceptanceCopy}>{acceptanceStatement}</Text>
            {note.signature ? (
              <View style={styles.signatureRow}>
                <Image
                ...
                <View style={styles.signatureMeta}>
                  ...
                  <Text style={styles.signerSub}>
                    Signed {date(note.signature.signed_at, true)}
                  </Text>
                </View>
              </View>
            ) : (
...
export async function downloadReport(
  ...
  await validateReportAssets(note);
  const { pdf } = await import("@react-pdf/renderer");
  const blob = await pdf(
    <ReportDocument note={note} organization={organization} />,
  ).toBlob();
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  const safeNumber = (note.service_number || "service-report").replace(
    /[^a-z0-9_-]+/gi,
    "-",
  );
  link.href = url;
...
type ReportAssetLoader = (source: string) => Promise<void>;
const browserAssetLoader: ReportAssetLoader = (source) =>
...
export async function validateReportAssets(
  note: ServiceNote,
  loader: ReportAssetLoader = browserAssetLoader,
) {
  const assets = [
    ...note.photos.map((photo) => photo.url),
    ...(note.signature ? [note.signature.image] : []),
  ];
  for (const source of assets) {
    if (!source?.trim()) throw new Error("A report image could not be loaded.");
    if (
      source.startsWith("data:image/") &&
      !/^data:image\/(?:png|jpeg);base64,/i.test(source)
    )
      throw new Error("A report image uses an unsupported format.");
    try {
      await loader(source);
    } catch {
