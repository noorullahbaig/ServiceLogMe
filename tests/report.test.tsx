...
import { ServiceReport } from '../src/components/service-report';
import { ReportDocument } from '../src/lib/report-pdf';
import type { Organization, ServiceNote } from '../src/lib/types';

const pixel = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M/wHwAEAQH/2mVTjwAAAABJRU5ErkJggg==';
...
export const completedReportNote: ServiceNote = {
  ...
  work_performed: 'Inspected the regulator, replaced the seal kit, and pressure tested the assembly.',
  result_remarks: 'Pressure remained stable at the specified operating range.',
  additional_notes: 'Recommend a follow-up inspection during the next planned shutdown.',
  ...
  labor: [{ id: 'labor-1', name: 'Aina Rahman', classification: 'Senior Service Engineer', hours: '2.5', rate: '80', notes: 'Inspection and testing' }],
  materials: [{ id: 'material-1', description: 'Seal kit', part_number: 'SK-20', quantity: '1', unit_amount: '45', photo_id: 'photo-1' }],
  charges: [{ id: 'charge-1', description: 'Travel', amount: '50' }],
  photos: [{ id: 'photo-1', url: pixel, category: 'AFTER', caption: 'Assembly after pressure test', created_at: '2026-09-09T04:20:00Z', name: 'assembly.png' }],
  signature: { signer_name: 'Daniel Lee', signer_position: 'Plant Manager', image: pixel, signed_at: '2026-09-09T05:00:00Z' },
  labor_total: '200.00',
...
describe('service report', () => {
  it('renders the complete customer-facing record without editable controls', () => {
    const html = renderToStaticMarkup(<ServiceReport note={completedReportNote} organization={reportOrganization} />);
  ...
  it('generates a real PDF document from the same completed note', async () => {
    // Image decoding is an upstream renderer concern. This keeps the PDF
    // contract focused on the report's own structured content while browser
    // coverage verifies that photos and signatures render in the screen report.
    const reportWithoutRasterAssets = {
      ...completedReportNote,
      photos: [],
      signature: null,
    };
    const buffer = await renderToBuffer(<ReportDocument note={reportWithoutRasterAssets} organization={reportOrganization} />);
